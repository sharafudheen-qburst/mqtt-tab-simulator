...
public sealed class InboundMessageStore
{
    ...
    public InboundMessageStore(SimulatorDatabase database)
    {
    ...
    public long GetMaxSequence() =>
        _database.Execute(connection =>
        {
            ...
            command.CommandText = "SELECT COALESCE(MAX(sequence), 0) FROM inbound_messages;";
        ...
        _database.Execute(connection =>
        {
            ...
            command.CommandText =
                ...
                INSERT OR REPLACE INTO inbound_messages (
                  sequence, received_at, topic, payload_length, retained,
                  decoded_summary, payload_hex, event_type, equipment_id
                ) VALUES (
                  $sequence, $receivedAt, $topic, $payloadLength, $retained,
    ...
    public int ImportMany(IEnumerable<TabletInboundMessage> messages)
    {
        var imported = 0;
        _database.Execute(connection =>
        {
    ...
    public int Count() =>
        ...
        {
            using var command = connection.CreateCommand();
            command.CommandText = "SELECT COUNT(*) FROM inbound_messages;";
    ...
    public void Clear()
    {
        ...
        {
            using var command = connection.CreateCommand();
            command.CommandText = "DELETE FROM inbound_messages;";
            command.ExecuteNonQuery();
        });
    }
    ...
    private static void BindMessage(SqliteCommand command, TabletInboundMessage message)
    {
        ...
        command.Parameters.AddWithValue("$receivedAt", message.ReceivedAt.ToString("O"));
